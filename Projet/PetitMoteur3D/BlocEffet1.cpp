...
void CBlocEffet1::Anime(float tempsEcoule)
{
	...
	matWorld = XMMatrixRotationRollPitchYaw(tm.q.x, tm.q.y, tm.q.z); 
...
void CBlocEffet1::InitEffet()
{
	...
	DXEssayer(D3DCompileFromFile(L"SimplePhong.fx", 0, 0, 0,
...
void CBlocEffet1::InitPhysique()
{
	...
	PxTransform localTm(PxVec3(10,10,10));
	body = rMoteur.Moteur_Physique.gPhysics->createRigidDynamic(localTm);
	body->attachShape(*shape);
	body->setMass(1.0f);
	PxRigidBodyExt::updateMassAndInertia(*body, 500.0f);
	body->setLinearVelocity(PxVec3(0, 1, 0.5), false); 

	rMoteur.Moteur_Physique.gScene->addActor(*body); 
}
