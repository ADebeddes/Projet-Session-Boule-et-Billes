...
bool CMoteurWindows::RunSpecific()
{
	...
	if (::PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE))
	{
		...
		if (!windowsIsActive)
		{
			GestionnaireDeSaisie.Unacquire();
		}

		if (windowsIsActive)
		{
			GestionnaireDeSaisie.Acquire(msg.hwnd);
		}
		...
		{
			GestionnaireDeSaisie.click = true;
		}
...
void CMoteurWindows::BeginRenderSceneSpecific()
{
	...
	float Couleur[4] = { 0.0f, 0.4f, 0.8f, 1.0f };  //  RGBA
...
void CMoteurWindows::UpdateSouris()
{
	GestionnaireDeSaisie.x = x;
	GestionnaireDeSaisie.y = y;
}

int CMoteurWindows::largeur;
int CMoteurWindows::hauteur;
void CMoteurWindows::UpdateEcran()
{
	pDispositif->setHauteur(hauteur);
	pDispositif->setLargeur(largeur);
	updateRatioEcran();
}
